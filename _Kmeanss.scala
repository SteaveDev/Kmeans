...
object KMeanss extends App {
  ...
  for(i<-0 to n_limite-1){
    ...
    for(j<-1 to dimension) mesPoints_temp.append(n_limite*(j*10)*i)      ////for(j<-1 to dimension) mesPoints_temp.append(random.nextInt(n_limite) + random.nextDouble())
